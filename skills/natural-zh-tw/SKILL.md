...
version: 1.6.0
description: |
  「去 AI 味」文章優化版：審查與改寫繁體中文內容，校正中國用語與半形標點，讓文字自然、具體且容易閱讀。
  觸發時機：用戶要求修改、改寫或潤飾文章，且提到「說人話」「去 AI 味」或「去AI味」時，一律觸發；也適用於「這段好 AI」「改自然一點」「幫我潤稿去掉 AI 感」「校對一下再發」，或要求檢查電子報、社群貼文、銷售頁、課程文案、客服回信、簡報、公告、Email 等對外文字的語感。
  不要觸發：逐字翻譯、模仿特定品牌 voice、事實查核（非風格問題）、程式碼／log／設定檔、要求「潤成雷蒙的語氣」（那是 content-writing skill 的事，本 skill 只去 AI 味、不加個人風格）。
user-invocable: true
maturity: governed
review_cadence: quarterly
last-updated: 2026-09-09
author: Terry（基於 Raymond Hou 的 MIT 授權作品優化）
tags: [writing, proofreading, zh-tw, de-ai, humanizer]
changelog:
  - "1.6.0 (2026-09-09): 更名為 natural-zh-tw，中文名稱改為「去 AI 味｜文章優化版」，保留原作授權與來源說明。"
  - "1.5.1 (2026-09-09): 明確加入文章修改搭配「說人話」「去 AI 味／去AI味」的自動觸發規則，並更新一般使用者 README 與 SEO 二次確認流程。"
  ...
  - "1.4.1 (2026-09-08): 依 Terry 的文章示範，新增語義分段與段落留白；同一論述保留多句，不逐句換行，交稿前檢查過密與過碎。"
...
使用者只要提出修改、改寫或潤飾文章的要求，並在同一個需求中提到「說人話」「去 AI 味」或「去AI味」，就啟用本 skill。這些是意圖觸發詞，不需要逐字符合固定句型；例如「幫我修改文章，說人話一點」「這篇請去 AI 味」「幫我把文章去AI味」都算。

如果使用者只是在詢問這些詞的意思，沒有要求處理文字，不要自動改稿。觸發後仍依下方規則判斷是先審閱、直接改寫或只標問題；觸發詞本身不代表可以略過使用者指定的交付方式。
...
**這條規則凌駕本文件其他所有輸出格式指示。不論是 Skill 被自動觸發、或使用者直接下 `/natural-zh-tw` 這類 command，只要分析後查到任何建議修改的地方，一律適用，沒有「稿子很短就直接改」這種例外。**
...
- **段落之間留一個空白行**：純文字與 Markdown 用一個空白行分隔語義段落；段內讓畫面自然折行，不手動按字數換行。Google Docs 等富文字文件使用原生段落與適當段後間距，呈現相當於一行的留白；不要同時疊加空段和大段距。使用者在文件內改好的範本優先於貼進聊天室後可能失真的換行；先讀取原生段落及段距，再套用。
- **不硬套句數或字數**：長段落先找語義轉換點，再決定是否分段；短段落若只是上一段的延續，就合併回去。使用者提供的示範優先於固定長度規則。
...
Terry 的分段示範（2026-09-08 已直接讀取「買中古車注意事項」Google Doc 前言確認）：第一段把找車情境、價格落差與疑問放一起；第二段解釋怎麼參考估價；第三段點出進車行前要做的準備。原生文件為三個正文段落，每段段後距離 11.25 pt，段落之間沒有額外空段；本批四篇沿用這個留白值。其他文件若有使用者指定的新範本，重新取樣，不把 11.25 pt 當成所有版型的硬性規定。以下數字只是使用者提供的排版示例，不是可引用的行情資料。
